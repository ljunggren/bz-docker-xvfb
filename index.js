  ...
  page.on('console', msg => {
    ...
    if (logString.includes("BZ-LOG")) {
      ...
      if (formattedLog.includes("list-scenarios")){
       ...
       // Handle list tests
       if (formattedLog.includes("list-suite")){
        let scenarios=logString.split("list-suite:")[1];
        console.log("Found vector: " + scenarios);
        let formattedScenarios = scenarios.replace(".","/");
        console.log("Found matching tests: " + formattedScenarios);
        fs.writeFile(`${file}.list`, formattedScenarios, (err) => {
          if (err) {
            console.error("Error: ", err)
            process.exit(2)
      ...
      // Handle ready
      else if (formattedLog.includes("ready")){
        console.log("Server ready");
        ready = true;
        page.evaluate(()=> {console.log("BZ-LOG: Trigger")} );
